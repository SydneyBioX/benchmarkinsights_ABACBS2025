...
paged_table(spatialsimbench_result)
...
Here we extract the benchmark result from BenchmarkInsights object

```{r}
paged_table(head(ssm_insight$evalSummary))
```
...
```{r}
paged_table(unique(ssm_insight$evalSummary[, c("evidence", "metric")]))
```
...
## 2.6 To what extent is the method sensitive to each metric?
